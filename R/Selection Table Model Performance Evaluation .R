...
for(a in 1:length(thresholds)){
  ...
  TopModelDetectionDF_single$PredictedClass <-  ifelse(TopModelDetectionDF_single$Probability  < thresholds[a], 'Noise','CrestedGibbons')
    ...
    # Create a row for the result and add it to the BestF1data.frame
    TrainingData <- training_data_type
    TempF1Row <- cbind.data.frame(F1caret, Precisioncaret, Recallcaret, TrainingData)
    TempF1Row$thresholds <- thresholds[a]
    BestF1data.frame <- rbind.data.frame(BestF1data.frame, TempF1Row)
  }

BestF1data.frame

# Convert TrainingData to a factor variable
BestF1data.frame$TrainingData <- as.factor(BestF1data.frame$TrainingData)

# Create a new column TrainingCategory based on TrainingData
BestF1data.frame$TrainingCategory <- ifelse(str_detect(BestF1data.frame$TrainingData, 'split') == TRUE,
                                            'split', 'not split')

# Set custom levels for TrainingData
levels(BestF1data.frame$TrainingData) <- c('20 calls', '20 calls', '40 calls', '40 calls',
                                           '5 calls', '5 calls', '65 calls', '65 calls')

# Reorder TrainingData levels
BestF1data.frame$TrainingData <- factor(BestF1data.frame$TrainingData,
                                        levels = c('5 calls', '20 calls', '40 calls', '65 calls'))

# Round Recall values to two decimal places
BestF1data.frame$Recallcaret <- round(as.numeric(BestF1data.frame$Recallcaret), 2)

# Create a line plot for F1 score vs. TrainingData, faceted by thresholds
ggline(data = BestF1data.frame, x = 'TrainingData', y = 'F1caret', color = 'TrainingCategory', facet.by = 'thresholds')

# Create a line plot for Recall vs. TrainingData, faceted by thresholds
ggline(data = BestF1data.frame, x = 'TrainingData', y = 'Recallcaret', color = 'TrainingCategory', facet.by = 'thresholds')

# Create a line plot for Precision vs. Recall, faceted by thresholds
ggline(data = BestF1data.frame, x = 'Recallcaret', y = 'Precisioncaret', color = 'TrainingCategory', facet.by = 'thresholds')

# Create another line plot for Precision vs. Recall, faceted by thresholds (duplicate)
ggline(data = BestF1data.frame, x = 'Recallcaret', y = 'Precisioncaret', color = 'TrainingCategory', facet.by = 'thresholds')
